...
// Integration hook. The default preserves the upstream UAV write exactly.
// Vulkan integrations can override this before including the header to add
// explicit bounds checks because out-of-range storage-image writes are not
// portable without robust image access.
#ifndef BEND_SSS_WRITE_OUTPUT
	#define BEND_SSS_WRITE_OUTPUT(OUTPUT, COORD, VALUE) OUTPUT[COORD] = VALUE
#endif
...
struct DispatchParameters
{
	...
	static bool EarlyOutPixel(struct DispatchParameters inParameters, int2 pixel_xy, float depth)
	{
		...
		// pixel_xy is intentionally unused by the default implementation.
		// A C-style (void) cast crashes some DXC SPIR-V compiler versions.
	...
	void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
	{
		...
		//write the result
		{
			...
			BEND_SSS_WRITE_OUTPUT(inParameters.OutputTexture, (int2)write_xy, result);
		}
	}
