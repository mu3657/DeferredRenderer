...
{
#if RTXGI_DDGI_BLEND_USE_ROOT_DISPATCH_BASE
    // DeferredRenderer updates a small contiguous probe batch each frame.
    // Some Vulkan drivers do not expose vkCmdDispatchBase's non-zero base in
    // DXC-generated SV_GroupID/SV_DispatchThreadID as this kernel expects.
    // Reuse the otherwise unused reduction root constants as an explicit
    // physical Texture2DArray group offset (X, Z, Y).
    uint3 dispatchBase = GetReductionInputSize();
    GroupID += dispatchBase;
    DispatchThreadID += dispatchBase * uint3(
